...
char *PHP_HL_keywords[] = {
   "__halt_compiler","break","clone","die",	"empty",    "endswitch",
   "final",	"global", "include_once","list","private",  "return",
   "try",	"xor",    "abstract","callable","const",    "do",
   "enddeclare","endwhile","finally","goto",    "instanceof","namespace",
   "protected", "static", "unset",   "yield",   "and",	    "case",
   "continue",  "echo",   "endfor",  "eval",    "for",      "if",
   "insteadof", "new",    "public",  "switch",  "use",      "array",
   "catch",     "declare","else",    "endforeach","exit",   "foreach",
   "implements","interface","or",    "require", "throw",    "var",
   "as",        "class",  "default", "elseif",  "endif",    "extends",
   "function",  "include","isset",   "print",   "require_once","trait",
   "while",     NULL};

char *JSON_HL_keywords[] = {NULL};

char *XML_HL_keywords[] = {NULL};

char *SQL_HL_keywords[] = {"SELECT",
                           "FROM",
                           "DROP",
                           "CREATE",
                           "TABLE",
...
int editorRowCxToRx(erow *row, int cx) {
  ...
  for (j = 0; j < cx; j++) {
    if (row->chars[j] == '\t')
      rx += (BARACLE_TAB_STOP - 1) - (rx % BARACLE_TAB_STOP);
    rx++;
  }
  return rx;
}
...
int editorRowRxToCx(erow *row, int rx) {
  ...
  for (cx = 0; cx < row->size; cx++) {
    if (row->chars[cx] == '\t')
      cur_rx += (BARACLE_TAB_STOP - 1) - (cur_rx % BARACLE_TAB_STOP);
    cur_rx++;
...
void editorUpdateRow(erow *row) {
  ...
  free(row->render);
  row->render = malloc(row->size + tabs * (BARACLE_TAB_STOP - 1) + 1);

  int idx = 0;
  for (j = 0; j < row->size; j++) {
    if (row->chars[j] == '\t') {
      row->render[idx++] = ' ';
      while (idx % BARACLE_TAB_STOP != 0)
        row->render[idx++] = ' ';
    } else {
...
void editorProcessKeypress() {
  static int quit_times = BARACLE_QUIT_TIMES;
